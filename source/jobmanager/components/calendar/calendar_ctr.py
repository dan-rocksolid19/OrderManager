...
class Calendar(ctr_container.Container):
    ...
    def show_day_jobs_dialog(self, date, jobs_for_day):
        ...
        try:
            ...
            if confirm_action(message, "Day Jobs"):
    ...
    def create_event(self, event):
        ...
        from librepy.jobmanager.components.calendar import events
    ...
    def create_job(self, event):
        ...
        try:
            from librepy.jobmanager.components.joblist import job
            dlg = job.Job(self.parent, self.ctx, self.smgr, self.frame, self.ps)
            result = dlg.execute()
            
            if result == 1:  # Job was created
                # Reload calendar data to show new job
                self.load_calendar_data()
                self._create_calendar_grid()
